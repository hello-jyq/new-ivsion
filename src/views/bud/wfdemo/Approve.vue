  ...
  <div class="addApply">
    <div class="content">
        ...
        <div class="input_box">
          <div class="flex3">
            ...
            <div class="input detail-label">
              {{ quotationSaveForm.quotationName }}
            </div>
          </div>
          <div class="flex3">
            <div class="title">
            ...
            <div class="input detail-label">
              <dict-write v-model="quotationSaveForm.quotationType" dict-type-id="QuoType" class="input" />
            </div>
          </div>
          <div class="flex3">
            <div class="title">
          ...
          <div class="flex3">
            ...
            <div class="input detail-label">
              {{ dateFtt("yyyy-MM-dd", quotationSaveForm.quotationCommitDate) }}
            </div>
          </div>
          <div class="flex1">
            <div class="title">
      ...
      <div v-if="editScreenFlag" class="submit">
        <!-- 编辑按钮 -->
        <el-button v-if="backto == null" type="primary" plain @click="handleEdit">
          {{ $t('comm.edit') }}
        </el-button>
        <!-- 删除按钮 -->
        <el-button v-if="backto == null" type="danger" plain @click="handleDelete">
          {{ $t('comm.delete') }}
        </el-button>
        <!-- 更新履历按钮 -->

        <!-- 流程履历 todo 审批画面的时候需要表示-->
        <el-button v-if="backto == null" type="info" plain @click="handleProcessInstClick">
          {{ $t('comm.applyHistory') }}
        </el-button>

        <el-button v-if="backto == null" type="info" plain @click="handleUpdateHistoryClick">
    ...
    <el-dialog id="processDialog" :title="$t('workflow.detailedProcess') + quotationSaveForm.workflowProcessInstId" :visible.sync="dialogWfHistoryVisible" width="80%">
      <process-detail :key="processInstId" :process-inst-id="quotationSaveForm.workflowProcessInstId" />
    </el-dialog>
    <el-dialog
...
export default {
  ...
  computed: {
    ...
    backto: function() { return this.$route.query.backto }
  },
  watch: {
  },
  created() {
  ...
  methods: {
    ...
    handleEdit() {
      ...
      // 是否审批中检查
      if (this.quotationSaveForm.workflowFlag != null && this.quotationSaveForm.workflowFlag === '1') {
        this.$message({
          type: 'warning',
