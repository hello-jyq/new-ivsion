        ...
        >
          <el-form-item prop="year">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">{{ $t('bud.bdm.year') }}</el-col>
              <el-col :span="16">
                <el-select
                  v-model="searchParam.params.year"
                  placeholder="请选择"
                  class="light-select"
                  :popper-class="this.theme=='Light' ? 'blueSelect' : ''"
                  clearable
                >
                  <el-option
          ...
          <el-form-item prop="name">
            <el-row type="flex" :gutter="5" justify="space-between">
              <el-col :span="8">{{ $t('bud.bdm.name') }}</el-col>
              <el-col :span="16">
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <a
                  class="link"
                  @click.prevent="handleNumClick(scope.row.id)"
            ...
            >
              <template slot-scope="scope">
                <dict-write dict-type-id="BudTemplateStatus" :value="scope.row.tplStatus" />
              </template>
            </el-table-column>
          </el-table>
        </div>
...
import { getBafList } from "@/api/bud/baf/baf-api.js";
...
export default {
  ...
  methods: {
    ...
    async getTplYear() {
      // 获取年度方法
      const res = await getTplYears({ funcDiv: "BAF" });
      if (res && res.success) {
    ...
    async fetchData() {
      const res = await getBafList(this.searchParam);
    ...
    //   }
    //   const selectItemId = ''
    //   this.$router.push({ path: '/baf/edit', query: {
    //     id: selectItemId
    //   }})
    // },
    handleEdit(multipleSelection) {
