    ...
    <el-collapse v-model="activeNames">
      ...
      <el-collapse-item name="2">
        <h3 slot="title" class="collapse-title">
          预算明细
          <div class="add_row iconfont icon-jia" @click.stop="addRows">新增一行</div>
        </h3>
        <div class="table">
  ...
  methods: {
    ...
    addBudgetList(row, list) {
      ...
      monthList.forEach((item, index) => {
        hot.hotInstance.setDataAtRowProp(
          row,
          item,
          list[index] === 0 ? null : list[index]
        );
      });
    },
    // 保存提交数据
    ...
    clearHandsontable() {
      this.$refs.textHot.hotInstance.alter(
        "remove_row",
        3,
        this.hotSettings.data.length - 3
      );
