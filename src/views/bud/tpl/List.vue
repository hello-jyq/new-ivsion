  ...
  <div ref="apply" class="apply" :class="theme=='Light' ? 'apply-light' : 'apply'">
        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          <el-form-item prop="year">
            <el-row type="flex" :gutter="5" justify="space-between">
              ...
              <el-col :span="16">
                <el-select v-model="searchParam.params.year" clearable placeholder="请选择" class="light-select" :popper-class="theme=='Light' ? 'blueSelect' : ''">
                  <el-option
      ...
      <div class="table-bg-box search-list-table">
        <div ref="tables" style="height: 100%;">
          <el-table ref="table" v-loading="isLoading" class="scroll-table-box" :data="dataList" height="100%" stripe border @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="identiferNum" :label="$t('bud.tpl.identiferNum')" min-width="100" align="center" header-align="center" show-overflow-tooltip fixed>
              <template slot-scope="scope">
                <a class="link" @click.prevent="viewTemplate(scope.row.id)">{{ scope.row.identiferNum }}</a>
              </template>
            </el-table-column>
            <el-table-column prop="name" :label="$t('bud.tpl.name')" min-width="200" header-align="center" show-overflow-tooltip fixed />
            <el-table-column prop="year" :label="$t('bud.tpl.year')" width="100" align="center" header-align="center" show-overflow-tooltip />
            <el-table-column prop="status" :label="$t('bud.tpl.status')" width="100" align="center" header-align="center" show-overflow-tooltip>
              <template slot-scope="scope">
                <dict-write dict-type-id="BudTemplateStatus" :value="scope.row.status" />
              </template>
            </el-table-column>
            <el-table-column prop="remark" :label="$t('bud.tpl.remark')" header-align="center" min-width="300" show-overflow-tooltip />
    ...
    >
      <div class="box">
        ...
        <upload action="/api/comm/excel/upload/TPL" :other-params="otherParams" :accept="accept" :multiple="false" :on-success="handleUploadSuccess" :on-error="handleUploadError" @finish="uploadDialog=false" />
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    viewTemplate(templateId) {
      // alert(templateId)
      this.$router.push({ path: '/tpl/detail', params: { templateId: templateId, flag: 3 }})
      // console.log('templateId', templateId)
    },
    async addTemplate() {
      // 新增传递id为-1
      this.$router.push({ path: '/tpl/edit', params: { templateId: '-1', flag: 1 }})
    ...
    editTemplate() {
      ...
      this.$router.push({ name: '/tpl/edit', params: { templateId: rowObj.id, flag: 2 }})
    },
    initTemplate() {
      // 校验不可多选
      if (this.multipleSelection && this.multipleSelection.length !== 1) {
      ...
      const rowObj = this.multipleSelection[0]
      if (rowObj.status !== '01' && rowObj.status !== '02') { // 仅草稿与发布状态可以财务预设
        this.$message({
          type: 'message',
    ...
    cancelTemplate() {
      ...
      const rowObj = this.multipleSelection[0]
      if (rowObj.status !== '01' && rowObj.status !== '02') { // 仅草稿与发布状态可以作废
        this.$message({
          type: 'message',
