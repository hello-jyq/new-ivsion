        ...
        <el-collapse-item name="1">
          ...
          <div class="basic">
            ...
            <div>
              ...
              <el-input v-model="department" class="input" placeholder="预算部门" />
            </div>
            <div>
              <div class="title">
                预算年度
        ...
        <el-collapse-item name="3">
          ...
          <div class="mode">
            ...
            <div class="month">
              ...
              <div class="input">
                <el-date-picker v-model="start" type="month" placeholder="选择月" />至<el-date-picker v-model="end" type="month" placeholder="选择月" />
              </div>
            </div>
          </div>
        </el-collapse-item>
      ...
      <el-table :data="tableList" style="width: 100%">
        <!-- <el-table-column type="expand" fixed>
          <template slot-scope="props">
            <div class="valueList">
              <div class="value_item" v-for="item in props.row.valueList" :key="item.title">
        ...
        <el-table-column width="60" fixed type="index" />
        <el-table-column width="120" label="年份" fixed prop="year" />
        <el-table-column fixed width="120" label="分类" prop="category" />
        <el-table-column label="部门" width="120" prop="department" />
        <el-table-column label="项目编号" width="120" prop="itemNumber" />
        <el-table-column label="项目名称" width="120" prop="entryName" />
        <el-table-column label="科目编号" width="120" prop="subjectNumber" />
        <el-table-column label="科目名称" width="120" prop="subjectName" />
        <el-table-column label="预算编号" width="120" prop="budgetNumber" />
        <el-table-column label="状态" width="120" prop="state" />
        <el-table-column label="申请编号" width="120" prop="applicationNumber" />
        <el-table-column label="属性" width="120" prop="attribute" />
        <el-table-column label="范围" width="120" prop="range" />
        <el-table-column label="相关性" width="120" prop="relevance" />
        <el-table-column label="分析科目" width="120" prop="analyticalSubjects" />
        <el-table-column
          v-for="item in countL"
          :key="item.title"
          :label="item.title"
          width="120"
          :prop="item.title"
...
export default {
  ...
  computed: {
    ...
    tableList() {
      if (this.count === 1) {
        return [
          ...this.summaryList.slice(
            (this.currentPage - 1) * this.pageSize,
            this.currentPage * this.pageSize
          ),
          this.getSummaries(this.countL)
        ]
      } else {
  ...
  methods: {
    ...
    async __getSummaryList(params) {
      const month =
        new Date(this.start).getMonth() + 1 > 9
          ? new Date(this.start).getMonth() + 1
          : '0' + (new Date(this.start).getMonth() + 1)
      const endMonth =
        new Date(this.end).getMonth() + 1 > 9
          ? new Date(this.end).getMonth() + 1
          : '0' + (new Date(this.end).getMonth() + 1)
      const start =
        parseInt(String(new Date(this.start).getFullYear()) + month) || 0
      const end =
        parseInt(String(new Date(this.end).getFullYear()) + endMonth) ||
        1000000
      console.log(start)
      console.log(end)
