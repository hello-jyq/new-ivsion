<template>
  <div ref="apply" class="apply" :class="theme=='Light' ? 'apply-light' : 'apply'">
        ...
        <el-form ref="form" class="search-form default-input" label-position="left" :model="searchParam.params">
          ...
          <el-form-item prop="status">
            <el-row type="flex" :gutter="5" justify="space-between">
              ...
              <el-col :span="16">
                <dict-select v-model="searchParam.params.status" dict-type-id="BudItemStatus" class="light-select" theme="theme" />
              </el-col>
            </el-row>
          </el-form-item>
        ...
        <div ref="tables" style="height: 100%;">
          <el-table ref="table" v-loading="isLoading" class="scroll-table-box" :data="itemList" height="100%" stripe border @selection-change="handleSelectionChange">
            <el-table-column
              ...
              fixed
            />
            <el-table-column prop="identiferNum" :label="$t('basedata.identiferNum')" min-width="100" header-align="center" align="center" fixed>
              <template slot-scope="scope">
                <a class="link" @click.prevent="viewItem(scope.row.id)">{{ scope.row.identiferNum }}</a>
              </template>
            </el-table-column>
            <el-table-column prop="itemName" :label="$t('basedata.itemName')" min-width="200" show-overflow-tooltip header-align="center" align="left" fixed />
            <el-table-column prop="itemNum" :label="$t('basedata.itemNum')" min-width="150" header-align="center" align="left" />
            <el-table-column prop="parentItemName" :label="$t('basedata.parentItemName')" min-width="200" show-overflow-tooltip header-align="center" align="left" />
            <el-table-column prop="itemType" :label="$t('basedata.itemType')" min-width="80" header-align="center" align="center" />
            <el-table-column prop="status" :label="$t('basedata.status')" min-width="60" header-align="center" align="center" />
            <el-table-column prop="remark" :label="$t('basedata.remark')" min-width="300" show-overflow-tooltip header-align="center" align="left" />
          </el-table>
        </div>
      </div>
    ...
    <!-- 科目新建编辑Dialog -->
    <el-dialog :key="dialogKey" class="content-dialog-box search-light" :close-on-click-modal="false" custom-class="dialog-drag fixed-search-btn-box" top="0" :title.sync="titleType" :visible.sync="dialogSearchVisible" width="60%">
      <edit-dailog
...
import { getItems, newItem, editItem, getItemGroups } from '@/api/bud/itm/itm-api'
