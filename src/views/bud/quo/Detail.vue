    ...
    <div class="content">
      ...
      <div class>
        <div class="input_box">
          <div class="flex3">
            ...
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.quotationName }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.quotationType') }}</div>
            <div class="input">
              <!-- <dict-select v-model="quotationSaveForm.quotationType" dict-type-id="QuoType" class="input" disabled /> -->
              <label class="input detail-label">{{ quotationSaveForm.quotationType }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.email') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.pipelineId }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.businessDepartment') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.orgLevel1 }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.department') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.orgLevel2 }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.pmUser') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.pmUser }}</label>
            </div>
            <!-- ID为：{{ valueId }} -->
          </div>
          <div class="flex3">
            ...
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.contiAmount }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.customerName') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.accountId }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.creditRank') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.creditRank }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.quotationCommitDate') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.quotationCommitDate }}</label>
            </div>
          </div>
          <div class="flex3">
            <div class="title">{{ $t('bud.quo.transactionsType') }}</div>
            <div class="input">
              <label class="input detail-label">{{ quotationSaveForm.transactionsType }}</label>
            </div>
          </div>
          <div class="flex3">
          ...
          <div class="flex1">
            ...
            <div class="input">
              <label
                ...
                style="height: 60px"
              >{{ quotationSaveForm.caseOutline }}</label>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="in">
        <div>
          ...
          <label class="input detail-label">{{ quotationSaveForm.createTime }}</label>
        </div>
        <div>
          <div class="title">{{ $t('admin.role.createBy') }}</div>
          <label class="input detail-label">{{ quotationSaveForm.createBy }}</label>
        </div>
        <div>
          <div class="title">{{ $t('bud.quo.updateTime') }}</div>
          <label class="input detail-label">{{ quotationSaveForm.updateTime }}</label>
        </div>
        <div>
          <div class="title">{{ $t('bud.quo.updateBy') }}</div>
          <label class="input detail-label">{{ quotationSaveForm.updateBy }}</label>
        </div>
        <div class="submit">
          <el-button type="primary" @click="backToQuotationEdit">{{ $t('comm.edit') }}</el-button>
          <el-button type="danger">{{ $t('comm.delete') }}</el-button>
...
import { saveQuoInfo, downloadFile, findById } from "@/api/bud/bud.js";
...
export default {
  ...
  created() {
    this.id = localStorage.getItem("detatilId");
    findById({ id: this.id }).then(res => {
      ...
      if (this.quotationSaveForm.contiAmount !== null) {
        const contiAmount = this.moneyFormat(
          this.quotationSaveForm.contiAmount
        );
        this.quotationSaveForm.contiAmount = contiAmount;
      }
      if (this.quotationSaveForm.quotationCommitDate !== null) {
        const commitDate = this.dateFormat(
          this.quotationSaveForm.quotationCommitDate
        );
        this.quotationSaveForm.quotationCommitDate = commitDate;
      }
    });
  },
  methods: {
    ...
    moneyFormat(contiAmount) {
      var num = contiAmount + "";
      if (num === "undefined" || this.num === "") {
        return num;
      }
      let reg = null;
