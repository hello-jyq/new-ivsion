  ...
  <div class="dict">
    <div class="search">
      <el-collapse v-model="activeNames">
        <el-collapse-item name="1">
          ...
          <div class="input_box">
            <div class="flex3">
              <div class="title">{{ $t('admin.i18n.keySeqNo') }}:</div>
              <el-input
                v-model="searchParam.params.keySeqNo"
                class="input"
                :placeholder="$t('admin.i18n.keySeqNo')"
                clearable
              />
            </div>
            <div class="flex3">
              <div class="title">{{ $t('admin.i18n.dataType') }}:</div>
              <dict-select
                v-model="searchParam.params.dataType"
                class="input"
                dict-type-id="I18nResourceType"
                :placeholder="$t('admin.i18n.dataType')"
                clearable
              />
            </div>
            <div class="flex3">
              <div class="title">{{ $t('admin.i18n.locale') }}:</div>
              <dict-select
                v-model="searchParam.params.locale"
                class="input"
                dict-type-id="Language"
                :placeholder="$t('admin.i18n.locale')"
                clearable
              />
            </div>
          </div>
    ...
    <div class="content">
      ...
      >
        ...
        />
        <el-table-column
          v-if="tableHeaderList[1].isShow"
          prop="dataType"
          :label="$t('admin.i18n.dataType')"
          min-width="80"
        >
          <template slot-scope="scope">
        ...
        <el-table-column
          v-if="tableHeaderList[2].isShow"
          prop="locale"
          :label="$t('admin.i18n.locale')"
          min-width="80"
        >
          <template slot-scope="scope">
      ...
      <div class="pagination">
        <div class="select_header">
          <el-popover width="260" trigger="hover" placement="top-start">
            <div class="check_box">
              <el-checkbox
                v-model="checkAll"
                :indeterminate="isIndeterminate"
                @change="handleCheckAllChange"
              >{{ $t('comm.selectAll') }}</el-checkbox>
              <div class="check_box_item">
                <el-checkbox
                  v-for="value in tableHeaderList"
                  :key="value.label"
    ...
    <div class="editDilog">
      ...
      >
        <div class="edit">
          ...
          <div class="input_box">
            <div class="flex3">
              <div class="title must">{{ $t('admin.i18n.keySeqNo') }}</div>
              <div class="input">
                <el-input
                  v-model="dialogObj.keySeqNo"
                  :placeholder="$t('admin.i18n.keySeqNo')"
                  class="input"
                  @blur="validator('keySeqNo')"
                />
                <span
                  ref="keySeqNo"
                  data-roles="required"
                  :data-value="dialogObj.keySeqNo"
                  class="message"
                />
              </div>
            </div>
            <div class="flex3">
              <div class="title must">{{ $t('admin.i18n.dataType') }}</div>
              <div class="input">
                <dict-select
                  v-model="dialogObj.dataType"
                  :clearable="false"
                  class="input"
                  dict-type-id="I18nResourceType"
                  @change="(value)=>{validator('dataType',value)}"
                />
                <span
                  ref="dataType"
                  data-roles="required"
                  :data-value="dialogObj.dataType"
                  class="message"
                />
              </div>
            </div>
            <div class="flex3">
              <div class="title must">{{ $t('admin.i18n.locale') }}</div>
              <div class="input">
                <dict-select
                  v-model="dialogObj.locale"
                  :clearable="false"
                  class="input"
                  dict-type-id="Language"
                  @change="(value)=>{validator('locale',value)}"
                />
                <span
                  ref="locale"
                  data-roles="required"
                  :data-value="dialogObj.locale"
                  class="message"
                />
              </div>
            </div>
            <div class="flex3">
              <div class="title">{{ $t('admin.i18n.nameI18n') }}</div>
              <div class="input">
                <el-input
                  v-model="dialogObj.nameI18n"
                  :placeholder="$t('admin.i18n.nameI18n')"
                  class="input"
                  @blur="validator('nameI18n')"
                />
                <span
                  ref="nameI18n"
                  data-roles="required"
                  :data-value="dialogObj.nameI18n"
                  class="message"
                />
              </div>
            </div>
    ...
    <el-dialog title="导入" :visible.sync="uploadDialog" width="400px" class="upload_zip">
...
export default {
  ...
  methods: {
    ...
    ...mapActions(["loginInfoAction", "refreshDictAndClearStore"])
  }
};
</script>
