<template>
  ...
  <div class="apply apply-left">
        ...
        <el-form ref="searchForm" :model="form" label-width="80px" label-position="left">
          <el-row type="flex" justify="space-between">
            <el-col :span="7">
              ...
              >
                <el-input
                  v-model="searchParam.params.jobGroupName"
                  :placeholder="$t('admin.scheduler.jobGroupName')"
                />
              </el-form-item>
            </el-col>
            <el-col :span="7">
              ...
              <el-form-item class="big-input" :label="$t('admin.scheduler.jobName')" prop="jobName">
                <el-input
                  v-model="searchParam.params.jobName"
                  :placeholder="$t('admin.scheduler.jobName')"
                />
              </el-form-item>
            </el-col>
            <el-col :span="7" />
          ...
          <div class="btns">
            ...
            <div class="dialog-btn-box">
              <div class="dialog-btn-right" @click="resetForm('searchForm')">
                <span class="circle-bigger-btn btn-default-color">
                  <i class="iconfont iconrefresh" />
                </span>
                <span class="dialog-btn-text">重&nbsp;置</span>
              </div>
              <div>
                <span class="circle-bigger-btn btn-light-color">
      ...
      <div class="createBox">
        <div class="input_box">
          <div class="flex2">
            ...
            <div class="input">
              <el-input
                v-model="schedulerInfo.jobGroupName"
                class="input"
                :placeholder="$t('admin.scheduler.jobGroupName')"
                @blur="validator('jobGroupName')"
              />
              <span
                ref="jobGroupName"
                data-roles="required"
                :data-value="schedulerInfo.jobGroupName"
                class="message"
              />
            </div>
          </div>
          <div class="flex2">
            ...
            <div class="input">
              <el-input
                v-model="schedulerInfo.jobName"
                class="input"
                :placeholder="$t('admin.scheduler.jobName')"
                @blur="validator('jobName')"
              />
              <span
                ref="jobName"
                data-roles="required"
                :data-value="schedulerInfo.jobName"
                class="message"
              />
            </div>
          </div>
          <div class="flex2">
            ...
            <div class="input">
              <el-input
                v-model="schedulerInfo.className"
                class="input"
                :placeholder="$t('admin.scheduler.className')"
                @blur="validator('className')"
              />
              <span
                ref="className"
                data-roles="required"
                :data-value="schedulerInfo.className"
                class="message"
              />
            </div>
          </div>
        ...
        <div class="input_box">
          ...
          <div class="flex2">
            ...
            <div class="input">
              <el-radio
                v-model="schedulerInfo.triggerType"
                label="Cron"
                @change="(value)=>{validator('triggerType',value)}"
              >
                <!-- 日历周期触发 -->
              ...
              <span
                ref="triggerType"
                data-roles="required"
                :data-value="schedulerInfo.triggerType"
                class="message"
              />
            </div>
          </div>
          <div v-if="schedulerInfo.triggerType === 'Simple'" class="flex2">
            ...
            <div>
              <el-input
                v-model="schedulerInfo.intervalSeconds"
                class="input"
                :placeholder="$t('admin.scheduler.interval')"
              />
              <span
                ref="intervalSeconds"
                data-roles="positiveInt"
                :data-value="schedulerInfo.intervalSeconds"
                class="message"
              />
            </div>
          </div>
          <div v-if="schedulerInfo.triggerType === 'Cron'" class="flex2">
            ...
            <el-input
              v-model="schedulerInfo.cronExpression"
              class="input"
              placeholder="请输入Cron表示式"
            />
          </div>
        </div>
        <div class="input_box">
        ...
        <div v-for="(value,key,index) in schedulerInfo.jobPara" :key="index" class="input_box">
          <div class="flex2">
            <div class="title">{{ $t('admin.scheduler.parameterName') }}</div>
            <el-input
              class="input"
              :value="key"
              disabled
              :placeholder="$t('admin.scheduler.parameterName')"
            />
          </div>
          <div class="flex2">
            <div class="title">{{ $t('admin.scheduler.parameterValue') }}</div>
            <div class="input">
              <el-input
                class="input"
                :value="value"
                disabled
                :placeholder="$t('admin.scheduler.parameterValue')"
              />
              <el-button
                size="mini"
                icon="el-icon-delete"
    ...
    <el-dialog title="增加参数" :visible.sync="showAddVarDialog" width="50%" class="createDialog">
      <div class="input_box">
        <div class="flex2">
          <div class="title">{{ $t('admin.scheduler.parameterName') }}</div>
          <div class="input">
...
import {
  ...
  saveScheduler,
  execScheduler
} from "@/api/admin/scheduler-api.js";
export default {
  ...
  methods: {
    ...
    async fetchData() {
      ...
      if (res && res.success) {
        ...
        this.schedulerList = this.totalList.slice(
          (pageNo - 1) * this.searchParam.pageSize,
          pageNo * this.searchParam.pageSize
        );
      }
